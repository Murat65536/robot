...
public final class SwerveDrive extends SubsystemBase {
    ...
    public SwerveDrive() {
        ...
        swerveDriveSim =
                new SwerveDriveSim(
                        DriveConstants.DRIVE_FF,
                        DCMotor.getFalcon500(1),
                        DriveConstants.DRIVE_GEAR_RATIO,
                        DriveConstants.WHEEL_DIAMETER / 2.0,
                        DriveConstants.STEER_FF,
                        DCMotor.getFalcon500(1),
     ...
     * states and using {@link #setModuleStates(SwerveModuleState[], boolean, boolean)}.
     *
    ...
    public void resetPose(Pose2d pose, boolean resetSimPose) {
        if (resetSimPose) {
            ...
            // we shouldn't realistically be resetting pose after startup, but we will handle it anyway for
            // testing
            for (SwerveModule swerveMod : swerveMods) {
                swerveMod.simulationUpdate(0, 0, 0, 0, 0, 0);
            }
    ...
    public void simulationPeriodic() {
        ...
        List<Matrix<N2, N1>> driveStates = swerveDriveSim.getDriveStates();
        List<Matrix<N2, N1>> steerStates = swerveDriveSim.getSteerStates();
        totalCurrentDraw = 0;
