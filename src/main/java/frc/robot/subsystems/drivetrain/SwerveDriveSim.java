...
public class SwerveDriveSim {
    ...
    public void update(double dtSeconds) {
        Pair<Matrix<N2, N2>, Matrix<N2, N1>> driveDiscAB = Discretization.discretizeAB(drivePlant.getA(), drivePlant.getB(), dtSeconds);
        Pair<Matrix<N2, N2>, Matrix<N2, N1>> steerDiscAB = Discretization.discretizeAB(steerPlant.getA(), steerPlant.getB(), dtSeconds);
        ...
        for (int i = 0; i < numModules; i++) {
            ...
            steerStates.set(i, calculateX(steerDiscAB.getFirst(), steerDiscAB.getSecond(), steerStates.get(i),
                    steerInputs[i], steerKs));
            double currSteerStatePos = steerStates.get(i).get(0, 0);
            moduleDeltas[i] = new SwerveModulePosition(currDriveStatePos - prevDriveStatePos,
                    new Rotation2d(currSteerStatePos));
        }
    ...
    public List<Matrix<N2, N1>> getDriveStates() {
        ...
        for (Matrix<N2, N1> driveState : driveStates) {
            states.add(driveState.copy());
        }
        return states;
    }
    ...
    public List<Matrix<N2, N1>> getSteerStates() {
        List<Matrix<N2, N1>> states = new ArrayList<>();
        for (Matrix<N2, N1> steerState : steerStates) {
            states.add(steerState.copy());
        }
        return states;
    }
    ...
    public double[] getDriveCurrentDraw() {
        ...
        for (int i = 0; i < numModules; i++) {
            ...
            currents[i] = getCurrentDraw(driveMotor, radiansPerSec, driveInputs[i],
    ...
    public double[] getSteerCurrentDraw() {
        ...
        for (int i = 0; i < numModules; i++) {
            ...
            currents[i] = getCurrentDraw(steerMotor, radiansPerSec, steerInputs[i],
                    RobotController.getBatteryVoltage());
        }
        return currents;
    }
