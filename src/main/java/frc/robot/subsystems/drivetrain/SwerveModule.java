...
public class SwerveModule {
    ...
    private final PIDController drivePidController = new PIDController(DriveConstants.DRIVE_KP, DriveConstants.DRIVE_KI, DriveConstants.DRIVE_KD);
    // (A profiled steering PID controller may give better results by utilizing
    // feedforward.)
    private final PIDController steerPidController = new PIDController(DriveConstants.STEER_KP, DriveConstants.STEER_KI, DriveConstants.STEER_KD);
    ...
    public SwerveModule(ModuleConstants moduleConstants) {
        ...
        driveEncoder.setDistancePerPulse(DriveConstants.DRIVE_DIST_PER_PULSE);
        ...
        steerEncoder.setDistancePerPulse(DriveConstants.STEER_RAD_PER_PULSE);
    ...
    public void periodic() {
        ...
        double steerPid = steerPidController.calculate(
        ...
        double driveFF = DriveConstants.DRIVE_FF.calculate(desiredState.speedMetersPerSecond);
        double drivePid = 0;
        if (!openLoop) {
            // Perform PID feedback control to compensate for disturbances
            drivePid = drivePidController.calculate(driveEncoder.getRate(),
    ...
    /**
     * {@link SwerveModuleState} describing absolute module rotation and velocity in
     * meters per
     * second.
     */
    ...
    /**
     * {@link SwerveModulePosition} describing absolute module rotation and position
     * in meters.
     */
    ...
    /**
     * Constants about this module, like motor IDs, encoder angle offset, and
